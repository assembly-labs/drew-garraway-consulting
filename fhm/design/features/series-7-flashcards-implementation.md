...
*Building on SIE flashcard infrastructure with spaced repetition from day one*
...
const S7Progress = (function() {
    ...
    function getTopicStats(topicId, cards) {
        ...
        return {
            ...
            learning,
            new: newCards,
            percentage: topicCards.length > 0
                ? Math.round((mastered / topicCards.length) * 100)
                : 0
        };
    }
    ...
    function getOverallStats(allCards) {
        const data = getData();
        let mastered = 0, learning = 0, newCards = 0, due = 0;
...
const S7Session = (function() {
    ...
    function buildQueue(config) {
        ...
        switch (config.mode) {
            ...
            case 'weak':
                cards = S7Data.getByTopic(config.topic)
                    .filter(c => progress[c.id] && progress[c.id].box < 3);
                break;
            default:
                cards = S7Data.getByTopic(config.topic);
        }
    ...
    function getProgress() {
        return {
            ...
            percentage: stats.total > 0
                ? Math.round(((currentIndex) / stats.total) * 100)
                : 0
        };
    }
...
*Implementation Plan v1.0 - January 2024*
