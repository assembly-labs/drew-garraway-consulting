 ...
 *  - Media Session API for lock-screen metadata and controls
 *  - Sticky mini-player when scrolling away from active player
 *
...
(function () {
    ...
    const STORAGE_PREFIX = 'fhm_audio_';
    const activePlayers = [];
    let currentlyPlaying = null; // tracks the active player for mini-player
    ...
    function savePosition(audio) {
        try {
            const key = storageKey(audio.currentSrc || audio.querySelector('source').src);
    ...
    function getTrackTitle(audio) {
        ...
        const playerBox = audio.closest('.audio-player');
        if (card) {
            const el = card.querySelector('.audio-card__title');
            return el ? el.textContent.trim() : '';
        }
        if (playerBox) {
            const el2 = playerBox.querySelector('.audio-player__title');
    ...
    function updateMediaSession(audio) {
        ...
        navigator.mediaSession.metadata = new MediaMetadata({
            title: getTrackTitle(audio),
            artist: 'Franklin Hugh Money',
            album: 'Securities Exam Prep',
        });
    ...
    function getMiniPlayer() {
        ...
        miniEl = document.getElementById('miniPlayer');
        if (!miniEl) return null;
        miniPlayBtn = miniEl.querySelector('.mini-player__play');
        miniTitle = miniEl.querySelector('.mini-player__title');
        miniFill = miniEl.querySelector('.mini-player__progress-fill');
        miniSpeedBtn = miniEl.querySelector('.mini-player__speed');
        miniCloseBtn = miniEl.querySelector('.mini-player__close');
        ...
        if (miniSpeedBtn) {
            miniSpeedBtn.addEventListener('click', function () {
                if (!currentlyPlaying) return;
                currentlyPlaying.speedIndex = (currentlyPlaying.speedIndex + 1) % SPEEDS.length;
                currentlyPlaying.audio.playbackRate = SPEEDS[currentlyPlaying.speedIndex];
                currentlyPlaying.speedBtn.textContent = SPEEDS[currentlyPlaying.speedIndex] + 'x';
                miniSpeedBtn.textContent = SPEEDS[currentlyPlaying.speedIndex] + 'x';
                savePosition(currentlyPlaying.audio);
            });
        }
        ...
        // Offset mini-player above SIE fixed bottom nav bar if present
        const sieNavBar = document.querySelector('.sie-nav-bar');
        if (sieNavBar && miniEl) {
            miniEl.style.bottom = sieNavBar.offsetHeight + 'px';
        }
            ...
            miniObserver = new IntersectionObserver(
                function (entries) {
                    entries.forEach(function (entry) {
                        const player = observedPlayers.get(entry.target);
                        if (player) player._visible = entry.isIntersecting;
                    });
    ...
    function showMiniPlayer(player) {
        if (!getMiniPlayer() || miniDismissed) return;
        if (miniTitle) miniTitle.textContent = getTrackTitle(player.audio);
        if (miniSpeedBtn) miniSpeedBtn.textContent = SPEEDS[player.speedIndex] + 'x';
        updateMiniPlayBtn(player.audio.paused);
    ...
    function updateMiniPlayBtn(paused) {
        if (!miniPlayBtn) return;
        miniPlayBtn.innerHTML = paused ? icons.play : icons.pause;
    ...
    function updateMiniPlayerVisibility() {
        ...
        if (currentlyPlaying._visible === false) {
    ...
    function observePlayer(player) {
        if (!getMiniPlayer() || !miniObserver) return;
        const target =
            player.wrapper.closest('.audio-card__player') ||
            player.wrapper.closest('.audio-player__controls') ||
            player.wrapper;
        observedPlayers.set(target, player);
        miniObserver.observe(target);
    }
    ...
    const icons = {
        ...
        skipBack:
            '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M11.99 5V1l-5 5 5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6h-2c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/><text x="12" y="15.5" text-anchor="middle" font-size="7" font-weight="700" font-family="Inter,sans-serif">15</text></svg>',
        skipForward:
            '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12.01 5V1l5 5-5 5V7c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6h2c0 4.42-3.58 8-8 8s-8-3.58-8-8 3.58-8 8-8z"/><text x="12" y="15.5" text-anchor="middle" font-size="7" font-weight="700" font-family="Inter,sans-serif">15</text></svg>',
    };
    ...
    function createPlayer(audio) {
        ...
        const saved = loadPosition(audio);
        if (saved) {
        ...
        audio.addEventListener('timeupdate', function () {
            ...
            if (currentlyPlaying === player) updateMiniProgress(audio);
        });
        ...
        // Register with mini-player IntersectionObserver
    ...
    function init() {
        getMiniPlayer(); // initialize mini-player DOM references
