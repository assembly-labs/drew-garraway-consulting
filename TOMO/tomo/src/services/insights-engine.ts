...
export function buildWeeklyInput(
  ...
  const weekPatterns = computeWeeklyPatterns(sessions, belt);
  const delta = computeWeeklyDelta(sessions, weekPatterns, belt);

  return {
    belt,
    userContext: userContextText,
    week: { start: weekPatterns.weekStart, end: weekPatterns.weekEnd },
...
export function checkInsightEligibility(
  ...
  const weeklyEligible = weekSessions.length >= 1 && !hasWeeklyThisWeek;
